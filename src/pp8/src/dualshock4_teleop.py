...
def controller_cb(msg):
    global pp8_l_mode, pp8_r_mode, l_break, r_break, l_wheel_vel, r_wheel_vel, l_arm_pos, r_arm_pos
    ...
    # Break.
    l_break = True if msg.btn_state.L1.data else False
    r_break = True if msg.btn_state.R1.data else False
    ...
    if (not abs(hat_x)) and (not abs(hat_y)):
        ...
        if pp8_l_mode == PP8_MODE.WHEEL:
            if l_break:
                ...
                l_wheel_vel = l_y if (abs(l_y) > AXIS_THRESH) else 0.0
        elif pp8_l_mode == PP8_MODE.ARM:
            l_wheel_vel = 0.0
            l_arm_pos = l_y if (abs(l_y) > AXIS_THRESH) else 0.0

        if pp8_r_mode == PP8_MODE.WHEEL:
            if r_break:
                r_wheel_vel = 0.0
            else:
                r_wheel_vel = r_y if (abs(r_y) > AXIS_THRESH) else 0.0
        elif pp8_r_mode == PP8_MODE.ARM:
            r_wheel_vel = 0.0
            r_arm_pos = r_y if (abs(r_y) > AXIS_THRESH) else 0.0
